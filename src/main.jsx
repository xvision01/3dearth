...
const aliases = {
  'unitedstatesofamerica': ['United States', 'USA'],
  'southkorea': ['South Korea', 'Korea, Republic of'],
  'northkorea': ['North Korea', "Korea, Democratic People's Republic of"],
  'czechrepublic': ['Czechia'],
  'ivorycoast': ["Côte d'Ivoire", 'Ivory Coast'],
...
function findCountryFact(geoName, countries) {
  ...
  const aliasMatch = countries.find((country) => (aliases[target] || []).some((name) =>
    allNames(country).some((candidate) => normalizeName(candidate) === normalizeName(name))
  ));
  if (aliasMatch) return aliasMatch;

  // Some GeoJSON names contain a longer political name while REST Countries uses a shorter common name.
  const fuzzy = countries.find((country) => allNames(country).some((name) => {
...
function CountryPanel({ country, geoName, loading, error, onClose }) {
  ...
  const currencies = country?.currencies
    ? Object.entries(country.currencies).map(([code, c]) => `${c.name || code}${c.symbol ? ` (${c.symbol})` : ''}`).join(', ')
    : '—';
    ...
    <aside className="country-panel" aria-live="polite">
      ...
      {loading ? (
        ...
        <div className="error-state">
          ...
          <p className="eyebrow">Country selected</p>
          <h2>{geoName}</h2>
          <p>We couldn't load the live country data right now. Please try selecting the country again.</p>
        </div>
      ) : (
        <>
          <div className="country-heading">
            ...
            <div>
              <p className="eyebrow">Country selected</p>
              <h2>{country.name?.common || geoName}</h2>
              <p className="muted">{country.name?.official || 'Official name unavailable'}</p>
            </div>
          </div>
          ...
          <div className="panel-footer">
            {country.tld?.[0] && <span>{country.tld[0]}</span>}
...
function App() {
  ...
  useEffect(() => {
    ...
    if (cached) {
      try { setCountryFactsData(JSON.parse(cached)); return; } catch { sessionStorage.removeItem('3dearth-country-facts'); }
    }
    ...
    fetch(COUNTRIES_URL)
      ...
      .then((data) => {
        ...
        setCountryFactsData(list);
        sessionStorage.setItem('3dearth-country-facts', JSON.stringify(list));
      })
  ...
  useEffect(() => {
    ...
    fetch(`https://restcountries.com/v3.1/name/${encodeURIComponent(geoName)}?fields=name,capital,region,subregion,population,area,languages,currencies,flags,maps,cca3,altSpellings,timezones,tld`)
      ...
      .then((data) => {
        const factFromSearch = Array.isArray(data) ? data[0] : null;
        if (!factFromSearch) throw new Error('Country not found');
        setCountryFacts(factFromSearch);
      })
      .catch((error) => { console.error(error); setFactsError(true); })
  ...
  const focusCountry = (polygon) => {
    setSelected(polygon);
    const center = polygon.properties?.center;
    if (center?.length === 2) {
      globeRef.current?.pointOfView({ lat: center[1], lng: center[0], altitude: 1.8 }, 850);
    }
  };

  const resetView = () => {
    globeRef.current?.pointOfView({ lat: 20, lng: 10, altitude: 2.4 }, 900);
    setSelected(null); setCountryFacts(null); setFactsError(false);
  };
  ...
  return (
    <main className="app-shell">
      ...
      <section className="hero-copy">
        ...
        <p className="subtitle">Drag the globe to rotate it freely. Hover over a country, then click to reveal reliable live facts.</p>
      </section>
